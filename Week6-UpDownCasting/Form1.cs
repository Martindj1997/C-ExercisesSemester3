...
namespace Week4B_BankAccountMgr
{
    public partial class Form1 : Form
    {
        ...
        {
            decimal interest = 0, overdraft = 0;
            int saCount = 0, caCount = 0;

            foreach (BankAccount b in branch.Accounts)
            {
                //SavingsAccount dangerousSA = (SavingsAccount)b;  // unsafe downcast
                ...
                if (b is ChequingAccount ca)
                {
                    if (ca.Balance < 0)
                        overdraft += ca.OverdraftFee;
                    caCount++;
                }
            }

            saTB.Text = saCount.ToString();
            caTB.Text = caCount.ToString();
            overTB.Text = overdraft.ToString("C");
            interestTB.Text = interest.ToString("C");
        }
    }
}
